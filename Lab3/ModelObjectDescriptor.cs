...
namespace GrafikaSzeminarium
{
    internal class ModelObjectDescriptor:IDisposable
    {
        ...
        private static void setNormals(float[] vertexArray, Vector3D<float> leftNormal, Vector3D<float> rightNormal)
        {
            int[] left = [0, 1];
            int[] right = [2, 3];
            foreach (var i in left)
            {
                vertexArray[i * 6 + 3] = leftNormal.X;
                vertexArray[i * 6 + 4] = leftNormal.Y;
                vertexArray[i * 6 + 5] = leftNormal.Z;
            }
            foreach (var i in right)
            {
                vertexArray[i * 6 + 3] = rightNormal.X;
                vertexArray[i * 6 + 4] = rightNormal.Y;
                vertexArray[i * 6 + 5] = rightNormal.Z;
            }
        }

        public unsafe static ModelObjectDescriptor CreateCube(GL Gl, Vector3D<float> leftNormal, Vector3D<float> rightNormal)
        {
            ...
            float[] vertexArray = new float[] {
                -0.25f, 0.5f, 0f, 0f, 0f, 1f,
                -0.25f, -0.5f, 0f, 0f, 0f, 1f,
                0.25f, -0.5f, 0f, 0f, 0f, 1f,
                0.25f, 0.5f, 0f, 0f, 0f, 1f
            };

            setNormals(vertexArray, leftNormal, rightNormal);
