...
namespace GrafikaSzeminarium
{
    internal class Program
    {
        ...
        private static ModelObjectDescriptor[] tiles;

        private static ModelObjectDescriptor[] badTiles;
        ...
        private const int numberOfTiles = 18;
        ...
        public static Vector3D<float> rotateY(Vector3D<float> vect, double angle)
        {
            float sinAngle = (float)Math.Sin(angle);
            float cosAngle = (float)Math.Cos(angle);
            float newX = vect.X * cosAngle + vect.Z * sinAngle;
            float newY = vect.Y;
            float newZ = -vect.X * sinAngle + vect.Z * cosAngle;
        ...
        private static void GraphicWindow_Load()
        {
            ...
            tiles = new ModelObjectDescriptor[18];
            for(int i = 0; i < numberOfTiles; i++)
            {
                tiles[i] = ModelObjectDescriptor.CreateCube(Gl, new Vector3D<float>(0, 0, 1), new Vector3D<float>(0, 0, 1));
            }
            badTiles = new ModelObjectDescriptor[18];
            for (int i = 0; i < numberOfTiles; i++)
            {
                badTiles[i] = ModelObjectDescriptor.CreateCube(Gl, rotateY(new Vector3D<float>(0, 0, 1), -Math.PI / 18), rotateY(new Vector3D<float>(0, 0, 1), Math.PI / 18));
            }
            ...
            Gl.ShaderSource(vshader, GetEmbeddedResourceAsString("Shaders.VertexShaderGourard.vert"));
            Gl.CompileShader(vshader);
            Gl.GetShader(vshader, ShaderParameterName.CompileStatus, out int vStatus);
            if (vStatus != (int)GLEnum.True)
                throw new Exception("Vertex shader failed to compile: " + Gl.GetShaderInfoLog(vshader));

            Gl.ShaderSource(fshader, GetEmbeddedResourceAsString("Shaders.FragmentShaderGourard.frag"));
        ...
        private static unsafe void GraphicWindow_Render(double deltaTime)
        {
            ...
            var tileScale = Matrix4X4.CreateScale((float)cubeArrangementModel.CenterCubeScale);
            var rotDiff = (float)Math.PI / 9;
            var tileRotation = Matrix4X4.CreateRotationY(rotDiff);
            var prev = 0f;
            for (int i = 0; i < numberOfTiles; i++)
            {
                float angle = 2 * i * (float)Math.PI / numberOfTiles;
                float rotationAngle = prev;
                Console.WriteLine(rotationAngle * (180f / (float)Math.PI));
                tileRotation = Matrix4X4.CreateRotationY(rotationAngle);
                var translation = Matrix4X4.CreateTranslation(radius * (float)Math.Sin(angle), 0f, radius * (float)Math.Cos(angle));
                var translUp = Matrix4X4.CreateTranslation(3f, 0f, 0f);
                var modelMatrix =  tileScale * tileRotation * translation;
                SetModelMatrix(modelMatrix);
                DrawModelObject(tiles[i]);
                var badModelMatrix = tileScale * tileRotation * translation * translUp;
                SetModelMatrix(badModelMatrix);
                DrawModelObject(badTiles[i]);
                prev += rotDiff;
            }
